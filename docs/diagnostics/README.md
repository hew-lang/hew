...
| ---------------------------------- | ---------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `E_LIMIT_COLLECTION_COPY`          | Limitation | v0.7.0     | A `Vec`/`HashMap`/`HashSet` binding moved (a rebind, a tuple/record/enum/array element, ...) and then read again. The rule says retain; this lowering consumes. Relabelled at the MIR site, not re-derived in the checker.                          |
...
| `E_LIMIT_SKIP_LEVEL_CAPTURE`       | Limitation | v0.7.0     | A closure capturing a binding from further out than its own immediately enclosing closure. Each closure's capture set is checked in isolation; the checker never propagates a nested closure's capture upward into its enclosing closure's own set. |
