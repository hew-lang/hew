...
| `Array` | element class joined with `CowValue` | `DeepCopy` for bit-copy elements; otherwise `FieldWise` or `None` as the element requires | Owns fixed-length element storage; native arrays use compact heap-backed recipes, preserving their exact `[T; N]` type and index-order element cleanup |
