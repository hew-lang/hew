...
mod tests {
    ...
    ///
    /// `hew_jwt_last_error_code` returns `i32` on the Rust side (`#[no_mangle]`
    /// above). The Hew binding in `jwt.hew` must declare the same return
    /// type, or the compiled call reads the error code through a mismatched
    /// ABI width. This test parses the `.hew` extern block and fails closed
    /// if the declared return type is anything other than `-> i32`.
    #[test]
    fn hew_binding_declares_jwt_last_error_code_returns_i32() {
        let hew_src = include_str!("../../std/crypto/jwt/jwt.hew");

        let decl = hew_src
            .lines()
            .map(str::trim)
            .find(|line| line.starts_with("fn hew_jwt_last_error_code"))
            .expect("jwt.hew must declare an extern `fn hew_jwt_last_error_code`");

        let signature = decl.split("//").next().unwrap_or(decl).trim();

        assert!(
            signature.contains("-> i32"),
            "jwt.hew binding for hew_jwt_last_error_code must return i32 to \
    ...
    /// FFI signature-parity guard (`uuid.rs`-style scalar pin).
    ///
    /// `hew_jwt_validate` returns `i32` on the Rust side (`#[no_mangle]`
    /// above). The Hew binding in `jwt.hew` must declare the same return
    /// type, or the compiled call reads the validation result through a
    /// mismatched ABI width. This test parses the `.hew` extern block and
    /// fails closed if the declared return type is anything other than
    /// `-> i32`.
    #[test]
    fn hew_binding_declares_jwt_validate_returns_i32() {
        let hew_src = include_str!("../../std/crypto/jwt/jwt.hew");

        let decl = hew_src
            .lines()
            .map(str::trim)
            .find(|line| line.starts_with("fn hew_jwt_validate"))
            .expect("jwt.hew must declare an extern `fn hew_jwt_validate`");

        let signature = decl.split("//").next().unwrap_or(decl).trim();

        assert!(
            signature.contains("-> i32"),
            "jwt.hew binding for hew_jwt_validate must return i32 to match \
