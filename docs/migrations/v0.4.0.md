...
> **v0.5 update (added post-release).** The `int` alias described in §1218
> below was a v0.4 transition surface and **was removed in v0.5**. The
> canonical integer type is `i64` (or the explicit-width `i32`/`i16`/etc. at
> ABI seams). If you are migrating directly to v0.5, replace `int` with `i64`
> at stdlib call sites rather than following the v0.4 migration below; the
> v0.5 stdlib never exposes `int`. See
> [`docs/specs/HEW-SPEC-2026.md`](../specs/HEW-SPEC-2026.md) §3 for the
> current type surface.

> **Forward-looking note (added post-release).** The explicit `close()` /
> `free()` migrations recorded below for `http.Server`, `http.Request`,
> `regex.Pattern`, and `json.Value` (#1314, #1500) are the **transition state
> for v0.4.x, not the destination.** They shipped to stop a documented
> double-free against FFI free functions, and the contract they created —
> "the user types the release call exactly once" — is the supported v0.4.x
> behaviour. The destination, specified in
> [`docs/specs/handle-safety-and-resource-lifetime.md`](../specs/handle-safety-and-resource-lifetime.md),
> is automatic single-release at scope exit with no `close()` / `free()` in
> user code, no garbage collector, and no parallel ownership oracle in
> codegen. That work is sequenced behind move-checker, owned-handle-validator,
> and `#1295` receiver-semantics resolution; it does not land in v0.4.x.
> **Do not infer from this note that v0.4.0 already has the destination
> model — it does not. The manual `free()` / `close()` lines below remain
> required on every v0.4.x exit path or the handle leaks.** Each per-handle
> migration in v0.5.x will prepend a "no longer required, see <link>" note to
> its entry below as it ships; until that note is present for a given
> handle, that handle is still manual-release.
...
fn main() {
    let server = http.listen(":8080");  // pre-0.4 signature: returned Server directly
    // ... handle requests ...
...
1. Search for every `http.listen(...)` call site. `listen` returns
   `Result<Server, net.NetError>`, so match on it and close only the `Ok`
   server; a bind failure now reports `Err` instead of handing back a
   null-backed handle. Before the binding goes out of scope (function return,
   actor receive handler end, loop iteration end), add `server.close()`.
...
The current release paths are `req.close()` / `val.free()`: requests are now
resources that close automatically, while JSON values remain manually freed.
...
1. Existing `req.free()` calls must become `req.close()`; explicit close is
   optional unless early release is desired.
...
Before the resource migration, omitting `req.free()` leaked each request
allocation. Current requests close automatically at handler scope exit.
