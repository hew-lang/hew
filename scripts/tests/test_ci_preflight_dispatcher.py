...
def test_a_third_tab_field_is_reported_not_folded_into_the_command() -> None:
    """A stray third tab field must not silently become part of the command.

    ``IFS=$'\\t' read -r seconds cmd`` puts everything past the first tab into
    ``cmd``, including a SECOND embedded tab -- so a row with a third field
    parsed as one command whose name nobody wrote and whose weight matched
    nothing real. That row is now reported the same way any other malformed
    row is: file, line, and the offending text, and the named command falls
    back to its timeout floor or the default rather than carrying the
    three-field row's number under a name it never had.
    """
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        corpus.write_text(
            ...
            f"{measured[target] * 2}\t{target}\textra-field\n",
            encoding="utf-8",
        )
        result = _run_dispatcher_process(
            ["bash", str(SCRIPT), "--dry-run", "--comprehensive", "--shard-plan", "4"],
        ...
        assert "extra-field" in result.stderr, result.stderr
        ...
        assert weights.get(target) != measured[target] * 2, (
            f"{target} carried the three-field row's weight: the extra "
            "field was folded into the command instead of rejecting the row"
        )
