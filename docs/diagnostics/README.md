...
| `E_SCOPE_IS_STATEMENT`                         | `scope { .. }` in a value position (§4.2). It brackets structured concurrency; batch `fork` (`await fork [ .. ]`) is the value-producing fan-out.                                                                                                                                 |
| `E_SEND_RESULT_DROPPED`                        | A statement-position send or ask whose typed delivery outcome is discarded (§2.1.1, §5.6). The fix-it writes `_ = send pid.m()` when the outcome is genuinely not wanted.                                                                                                         |
