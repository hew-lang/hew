...
// Lost coverage: `direct_record_projection_completes_parent_and_leaf_drop_authorities`
// used `--dump-mir raw`/`elab` (both retired) against a dedicated
// direct-vs-named record-projection fixture, pinning the retained
// field read-copy release, the provisional-parent completion, and
// exactly-one release on each mutually exclusive exit for both the
// anonymous call-result temporary and the named control. Physical
// MIR's structured (Debug) dump has no equivalent single-line text to
// grep or count per function, so this MIR-emission coverage has no
// direct replacement, and the fixture is not exercised by the
// leak-slope/malloc-scribble oracles below (they use a different,
// broader temporary-receiver fixture).
...
fn projection_shapes_source(frames: usize) -> String {
    format!(
        "fn inspect(values: Vec<string>) -> i64 {{\n\
         ...
         \x20   var map: HashMap<string, string> = HashMap.new();\n\
         \x20   var set: HashSet<string> = HashSet.new();\n\
         \x20   var values: Vec<string> = Vec.new();\n\
         \x20   var i = 0;\n\
         \x20   while i < 3 {{\n\
