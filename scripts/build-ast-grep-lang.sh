...
# Regenerate with the separately pinned tree-sitter CLI rather than trusting
# upstream's committed parser.c; that keeps the compiled dialect reproducible
# and auditable from grammar.js instead of an opaque pre-generated artefact.
(cd "$GRAMMAR_DIR" && "$TREE_SITTER" generate)
grep -q "#define LANGUAGE_VERSION $TREE_SITTER_HEW_LANGUAGE_ABI" "$GRAMMAR_DIR/src/parser.c" || {
    echo "error: pinned grammar ABI does not match tools/ast-grep.lock" >&2
    exit 1
}
...
*)
    echo "error: unsupported platform for custom ast-grep grammar: $(uname -s)" >&2
    exit 1
    ;;
esac
