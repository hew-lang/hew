...
**Status:** Spec — slice 1 of `lane-handle-safety-no-manual-free` (decision/design
deliverable, not implementation).
**Audience:** Hew language designers, stdlib maintainers, codegen and runtime
implementers, and reviewers gating subsequent migration slices.
...
§§ 7–8 lands and is calibrated on a single pilot handle (lane slice 2).
...
| E | Stdlib-level `OwnedHandle<T>` wrappers; user never sees the raw FFI handle. | Existing tier-2 owned-handle struct discipline. | **Required as transition substrate.** Pairs with C during slice-3 per-handle migration. |
...
- **Primary user-facing model:** **affine handle types (C) with codegen-emitted
  ...
  lane-1399 and the tier-1/tier-2 handle classification published by
  `feat-owned-handle-validator` (Phase 1 #1252, Phase 2 #1251).** The user
  writes no `close()` or `free()` line in normal code.
- **Stricter mode (B):** opt-in `#[must_consume]` (or equivalent annotation, to
  be named in slice 2) for stdlib-internal resources where dropping on the
  floor is a *correctness* bug, not a hygiene bug. Not exposed to general user
  code as the default.
...
This verdict is the input that gates lane slice 2 (calibration on a single
handle). If slice-2 calibration shows the affine model produces unacceptable
false positives, the verdict returns to discovery; slices 3+ may not begin
until the verdict survives calibration.
...
- A method declared on a tier-1 affine handle type takes its receiver
  **by-move**. After the call, the receiver binding is **null-after-move**
  (LESSONS `raii-null-after-move`); the move-checker (lane-1399) rejects any
...
- **Release methods (`close()`, `free()`):** ~9 (one per handle type listed in
  §6). These are the only methods that are *consuming* in v0.4.0 today, and
  they become redundant under the new model — the affine drop replaces them.
  They remain callable through slice 3 as a no-op or annotated as
  `#[deprecated]`, then are removed in v0.6.x.
...
The estimate that no more than **~9 stdlib methods** require a *signature*
change (the release methods) and that **~378 methods** only require
re-classification of an already-borrow-shaped call as borrow-shaped is the
signal that this resolution direction is migratable in the slice-3 cadence
(one PR per handle module).

A separate lane (`lane-1295-receiver-semantics`, to be filed) owns the parser,
checker, and codegen work to introduce the `consume` modifier and the
borrow-default reclassification. This spec records the *direction*; that lane
...
(lane-1399 substrate, codegen drop emission). It is the explicit answer to
slice-1 acceptance criterion #4.

---

## 6. Interaction with lane-1399 (single ownership model)

This lane and `lane-1399-move-checker-spec-prototype` publish **one** ownership
model, not two.

- **Lane-1399 owns:** the per-binding move/borrow analysis pass, its
  diagnostic vocabulary, its prototype implementation, and its acceptance /
  reject test fixtures. It is the *analysis substrate*.
- **This lane (handle safety) owns:** the user-facing language invariant
...
- **Shared:** the classification vocabulary (Owned / Borrowed / FieldAlias /
  ...
  serialised representation. Neither lane redefines these without
  cross-review by the other.
- **Disagreement protocol:** if a slice-3 per-handle PR cannot be expressed
  in lane-1399's vocabulary, the PR is held; lane-1399 amends its vocabulary
  *or* this spec amends its mechanism choice — but only after both lanes'
  reviewers concur. This spec does not silently introduce a parallel oracle.

This is the explicit answer to slice-1 acceptance criterion #5.

---

## 7. Per-handle tier assignment (the §1.1 inventory)

Every entry from the lane plan §1.1 is assigned a tier. Tiers determine which
mechanism applies and which slice-3 PR migrates the handle.

| Module | Type | Current release | Tier | Mechanism | Slice-3 order | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| `std/text/regex` | `Pattern` | `free()` | 1 — per-call resource | C (affine) + H (drop table) | **1 (calibration)** | Smallest blast radius; no streaming; no state machine. |
| `std/net/url` | `Url` | `free()` | 1 — per-call resource | C + H | 2 | Read-mostly; no streaming. |
| `std/encoding/json` | `Value` | `free()` | 1 — per-call resource (with field-alias hazard) | C + H, requires `field-alias-fail-closed` enforcement to be live | 3 | Recursive ownership: every `get_field`/`array_get` returns a new owned `Value`; affine drop of the parent must not fire while a child is live. Lane-1399 must publish FieldAlias before this PR ships. |
...
| Runtime actor / scheduler | implicit | `hew_runtime_cleanup` (`hew-runtime/src/scheduler.rs:459-490`) | 4 — runtime scope | I (session-reset registry) only; not affine | slice 4 | Process-scoped; the session-reset hook registry is the canonical teardown (LESSONS `cleanup-all-exits`). No user-facing release call; no per-binding affinity. |
| WASM mirror | parity | `hew-runtime/src/scheduler_wasm.rs` | 4 — runtime scope | I, parity-required | slice 4 | LESSONS `native-wasm-parity` mandates symmetric session_reset from `hew_sched_shutdown` paths in both lanes. See §10. |
...
Slice-3 migration order is "smallest blast radius first": tier 1 before tier
2; non-streaming before streaming; single-handle modules before multi-handle
modules. `regex.Pattern` ships first as the calibration (slice 2);
...
This is the explicit answer to slice-1 acceptance criterion #6.
...
| --- | --- | --- | --- |
| FM-1 | Double-free (explicit release + automatic drop both fire) | Checker (move-checker, lane-1399) | Reject at compile time; LESSONS `field-alias-fail-closed`, `raii-null-after-move`. |
...
| FM-4 | Leak — async cancel / future drop | Codegen + async cancel path | Cancellation runs the same scope-exit drop sequence as a normal return; LESSONS `cleanup-all-exits` "cleanup must mirror setup on every exit path". Slice-2 fixture `async_cancel_cleanup.hew` is mandatory. |
| FM-5 | Leak — actor shutdown / mailbox drain | Runtime session-reset hook (I) + per-actor fixture | `hew_sched_shutdown` calls the session-reset registry; tier-2/3 handles register a release entry. Slice-2 fixture `actor_shutdown_cleanup.hew` is mandatory. |
...
| FM-7 | Leak — panic unwind | Codegen drop emission on the unwind path | Same drop table as normal return; verified by panic-path fixture in slice 2. |
...
| FM-9 | WASM parity gap (native lane gets the cleanup, WASM lane does not) | `make test-wasm` per per-handle PR | LESSONS `native-wasm-parity`; either parity green or named `WASM-TODO` (§10). |
...
| FM-11 | Closure-capture leak (handle captured by long-lived closure that is never invoked) | Checker move-classification (`ClosureCapture`) | Lane-1399 publishes the classification; codegen emits a drop on closure deallocation. Reject if the captured handle's affine semantics cannot be satisfied. |
| FM-12 | Generator suspend/resume use-after-free (DROP-TODO D6 at `MLIRGen.cpp:5268`) | Checker move-classification (`YieldedAcrossSuspend`) | Lane-1399 must publish this category before any per-handle migration that touches a generator. Until then, generators that hold handle bindings are checker-rejected. |
| FM-13 | C ABI consumer double-free (C caller invokes `hew_*_free` after Hew already released) | C ABI test harness in `hew-cabi/tests/` | Slice-2 deliverable; release functions guarded by an FFI-side `closed` flag *only when proven idempotent and regression-tested* (LESSONS `ffi-ownership-contracts`). |

This is the explicit answer to slice-1 acceptance criterion #7.
...
- The **actor cleanup hook** (I) is duplicated across the two scheduler files.
  Slice 4 explicitly touches both. A per-handle slice-3 PR that introduces a
  new hook entry must update both schedulers in the same diff.
...
- **Explicit WASM-TODO carve-outs allowed in this lane:** none at spec time.
  If slice 2 calibration discovers a WASM-only gap (e.g. async cancel paths
  not yet implemented under `wasm32-wasip1`), the `WASM-TODO` is named in the
  calibration memo and tracked in a follow-on issue, *not* hidden by silent
  divergence.

---

## 11. Per-handle migration table (slice-3 work breakdown)

For every entry in §7, the slice-3 PR records what changes for the user, the
...
| `json.Value` | `val.free()` becomes a no-op; nested `get_field` results no longer require manual frees | tier-1 affine; FieldAlias must be live in lane-1399 | drop registered; field-alias pre-scan enforced |
...
Each PR includes the six fixtures listed in the lane plan §5.2 (`forget_free`,
`double_free`, `use_after_close`, `accept_no_manual_free`,
`actor_shutdown_cleanup`, `async_cancel_cleanup`) instantiated for that
handle.

This is the explicit answer to slice-1 acceptance criterion #8.

---

## 12. C ABI ownership tests (required validation surface)

C consumers cannot benefit from compile-time linearity proofs. The spec
chooses option (b) from the lane plan §7.7: **C ABI release functions are
wrapped in an idempotent FFI-side guard** *only after* the lower layer's
idempotence is proved and regression-tested. Until that proof exists for a
given handle, the C ABI release stays manual-close-only and the Hew side does
not auto-release for C-owned instances.

The test harness lives at **`hew-cabi/tests/`**. Each per-handle slice-3 PR
adds:
...
Without these tests passing for a given handle, that handle's slice-3 PR is
held. This is the explicit answer to slice-1 acceptance criterion #9.

---

## 13. Stop-the-bleeding rules (binding for v0.4.x)

Until tier-1 (per-call resource) discipline is settled by a successful slice-2
calibration *and* `feat-owned-handle-validator` Phase 1+2 has merged *and*
lane-1399 prototype has merged behind a flag, the following rules apply:
...
1. **No new stdlib handle types** that require user-visible manual `free()` /
   ...
   `// TEMPORARY: manual-free until handle-safety lane lands` comment **and**
   file a tracking GitHub issue that links this spec. The orchestrator gates
   PR review on (b) being explicit.
2. **No reintroduction of `impl Drop`** for any v0.4.0 handle type until the
   ...
   #1500.
3. **Examples teach the v0.4.0 contract until slice 3 ships per handle.**
   `examples/http_json_demo.hew` and any other example with manual `free()`
   lines is updated *in the same diff* as the per-handle migration PR that
   makes those lines no-ops, not before and not later.
4. **Audit pass on v0.4.x examples for missing frees** is in scope as a
   stop-the-bleeding task (lane plan §3.2 step 1). Each leak found becomes
   either an explicit `.free()` or an example rewrite that avoids the owned
   clone.
...
This is the explicit answer to slice-1 acceptance criterion #10.

---

## 14. References

- User directive: `lane-handle-safety-no-manual-free.md` §0–1.
- Migration record: `docs/migrations/v0.4.0.md` §§ #1314, #1500.
- Adjacent lanes: `lane-1399-move-checker-spec-prototype.md`,
  `feat-owned-handle-validator.md`, `feat-1228-runtime-handle-api.md`,
  `fix-http-server-close-drop-duplication.md`.
