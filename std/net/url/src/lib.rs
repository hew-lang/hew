...
/// Percent-encode a string (all characters that are not unreserved in RFC 3986).
///
/// Returns a `malloc`-allocated, NUL-terminated C string. The caller must free
/// it with `libc::free`. Returns null if `s` is null.
///
/// # Safety
///
/// `s` must be a valid NUL-terminated C string, or null.
#[no_mangle]
pub unsafe extern "C" fn hew_url_encode(s: *const c_char) -> *mut c_char {
    // SAFETY: If non-null, s is a valid NUL-terminated C string per caller contract.
    let Some(rust_str) = (unsafe { cstr_to_str(s) }) else {
        return std::ptr::null_mut();
    };
    let encoded = urlencoding::encode(rust_str).into_owned();
    str_to_malloc(&encoded)
}

/// Decode a percent-encoded string.
///
/// Returns a `malloc`-allocated, NUL-terminated C string. The caller must free
/// it with `libc::free`. Returns an empty string on decode error. Returns null
/// if `s` is null.
///
/// # Safety
///
/// `s` must be a valid NUL-terminated C string, or null.
#[no_mangle]
pub unsafe extern "C" fn hew_url_decode(s: *const c_char) -> *mut c_char {
    // SAFETY: If non-null, s is a valid NUL-terminated C string per caller contract.
    let Some(rust_str) = (unsafe { cstr_to_str(s) }) else {
        return std::ptr::null_mut();
    };
    match urlencoding::decode(rust_str) {
        Ok(decoded) => str_to_malloc(&decoded),
        Err(_) => str_to_malloc(""),
    }
}

/// Build a percent-encoded query string from a newline-separated list of
/// `key=value` pairs.
///
/// Each line in `params` should be `key=value`. Both key and value are
/// percent-encoded. Lines without `=` are skipped. The resulting pairs are
/// joined with `&`.
///
/// Returns a `malloc`-allocated, NUL-terminated C string. The caller must free
/// it with `libc::free`. Returns null if `params` is null.
///
/// # Safety
///
/// `params` must be a valid NUL-terminated C string, or null.
#[no_mangle]
pub unsafe extern "C" fn hew_url_encode_query(params: *const c_char) -> *mut c_char {
    // SAFETY: If non-null, params is a valid NUL-terminated C string per caller contract.
    let Some(rust_str) = (unsafe { cstr_to_str(params) }) else {
        return std::ptr::null_mut();
    };
    let encoded: Vec<String> = rust_str
        .lines()
        .filter_map(|line| {
...
mod tests {
    ...
    fn url_encode_basic() {
        ...
        let result = unsafe { hew_url_encode(input.as_ptr()) };
        // SAFETY: result is a valid malloc'd C string.
        let s = unsafe { read_and_free_cstr(result) };
        assert_eq!(s, "hello%20world");
    }

    #[test]
    fn url_encode_special_chars() {
        let input = CString::new("a=1&b=2").unwrap();
        // SAFETY: input is a valid NUL-terminated C string.
        let result = unsafe { hew_url_encode(input.as_ptr()) };
        // SAFETY: result is a valid malloc'd C string.
        let s = unsafe { read_and_free_cstr(result) };
        assert_eq!(s, "a%3D1%26b%3D2");
    }
    ...
    fn url_decode_basic() {
        ...
        // SAFETY: input is a valid NUL-terminated C string.
        let result = unsafe { hew_url_decode(input.as_ptr()) };
        // SAFETY: result is a valid malloc'd C string.
        let s = unsafe { read_and_free_cstr(result) };
    ...
    fn url_decode_invalid_sequence_passthrough() {
        // urlencoding::decode leaves invalid percent sequences as-is rather than erroring.
        // An invalid hex digit (%2Z) is passed through unchanged.
        let input = CString::new("bad%2Zseq").unwrap();
        // SAFETY: input is a valid NUL-terminated C string.
        let result = unsafe { hew_url_decode(input.as_ptr()) };
    ...
    #[test]
    fn url_decode_invalid_utf8_returns_empty() {
        // %FF is a valid percent sequence but not valid UTF-8 — decode should return "".
        let input = CString::new("%FF%FE").unwrap();
        // SAFETY: input is a valid NUL-terminated C string.
        let result = unsafe { hew_url_decode(input.as_ptr()) };
        // SAFETY: result is a valid malloc'd C string (empty on UTF-8 error).
        let s = unsafe { read_and_free_cstr(result) };
        assert_eq!(s, "");
    }
    ...
    fn url_encode_query_basic() {
        let input = CString::new("name=hello world\npage=1&2").unwrap();
        // SAFETY: input is a valid NUL-terminated C string.
        let result = unsafe { hew_url_encode_query(input.as_ptr()) };
        // SAFETY: result is a valid malloc'd C string.
        let s = unsafe { read_and_free_cstr(result) };
        assert_eq!(s, "name=hello%20world&page=1%262");
    }
}
