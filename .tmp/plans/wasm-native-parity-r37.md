...
- **Slice 5 (S9 trace + actor_type_id):** add tracing-on-wasm32 test that asserts non-zero `actor_type_id` after WASM codegen emits the registration (closes the `WASM-TODO` at `bridge.rs:154`, `tracing.rs:816`).
