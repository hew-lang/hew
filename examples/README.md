...
- **v05/surfaces/** -- One-file idiomatic demos for the landed v0.5 surfaces. Each ships a paired `.expected` and is gated by `make test-surface-examples`, except where noted below; that target fails on any surface example that has neither an `.expected` nor a justified entry in its `SURFACE_EXAMPLES_UNGATED` list, so a demo cannot go ungated by omission:
  ...
  - [`scanner_tokens.hew`](v05/surfaces/scanner_tokens.hew) -- line and word tokenisation through the value-state `std::io::scanner` API. **Currently excluded from `make test-surface-examples`** — it is the sole entry in that target's `SURFACE_EXAMPLES_UNGATED` list. Running it emits an obligation under-release advisory naming the loop-carried `Scanner` local, and the advisory is correct: the leak is in my drop elaboration, not in the example. The by-value owned-record escape scan behind `DropKind::RecordInPlace` cannot yet prove that a record passed by value into a value-state call (`scanner.next_line(sc) -> (Scanner, Option<string>)`) is not retained by the callee, so it fails closed and leaks the record's string fields rather than risk a double free. The example's Hew is the documented surface and stays as written; it goes back under the gate once the record drop prover clears this shape.
