...
This gate closes it, in five directions:
  ...
  A6  every gate-shaped comparison target either owns a baseline registered in
      `scripts/baselines.py` — so `make baselines` regenerates what it compares
      against — or carries a written reason for owning no derived artefact. A
      committed baseline whose only regen path is folklore is how ten of the
      thirty-five CI reds in the 2026-08-18..21 window happened.
...
# ── A6: derived baselines have exactly one regeneration path ──────────────────
#
# Ten of the thirty-five CI reds in the 2026-08-18..21 window were committed
