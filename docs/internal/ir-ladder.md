...
| `Ty::is_copy`, `vec_authority::is_copy_layout`; `ValueClass::Unknown` and its 15 sites; `hew-hir/src/value_class.rs` (moved); `BuiltinTypeMarker::Resource` on `LocalPid`/`HewActor`/`BoxedActor` and `None` on the BitCopy scalars/enums of §1.1 (markers corrected) | `hew-types/src/ty.rs:1452`, `vec_authority.rs:270`, `hew-hir`, `hew-types/src/builtin_type.rs:355` | P1 |
