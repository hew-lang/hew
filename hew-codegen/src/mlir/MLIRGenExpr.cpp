          ...
          if (scnIt != supervisorChildNames.end()) {
            ...
            for (size_t i = 0; i < childNameTypes.size(); ++i) {
              if (childNameTypes[i].first == fieldName) {
                ...
                auto timeoutVal = mlir::arith::ConstantIntOp::create(builder, location,
                                                                     builder.getI32Type(), 5000);
                return hew::RuntimeCallOp::create(
                           builder, location, mlir::TypeRange{ptrType},
...
mlir::Value MLIRGen::generateCallExpr(const ast::ExprCall &call, const ast::Span &exprSpan,
  ...
  if (callee && !callee.isExternal() && calleeName.substr(0, 4) != "hew_") {
    ...
    blockYieldsFieldAccess = [&](const ast::Block &blk) -> bool {
    ...
    exprYieldsFieldAccess = [&](const ast::Expr &expr) -> bool {
