...
`#[linear]` and `consume self` correctly (issue #1795, now resolved)
...
`consume self` (via `consuming_methods`) as of the #1795 fix.
...
- Codegen: emitting a binary that calls `close(consume self)` on scope
