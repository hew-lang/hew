...
#[test]
fn process_entry_callable_keys_preserve_complete_instance_identity() {
    let entry = hew_types::DefId::for_test("app.run_selected");
    let plan = EntryExitPlan {
        entry: entry.clone(),
        action: EntryExitAction::Unit,
    };
    assert_eq!(
        entry_body_callable_key(&plan),
        MirCallableKey::declared(entry)
    );

    let display = hew_types::DefId::for_test("app.Box::fmt");
    assert_eq!(
        entry_display_callable_key(&hew_types::EntryDisplayTarget {
            declaration: display.clone(),
            instance: hew_types::EntryCallableInstance::Declared,
        }),
        MirCallableKey::declared(display.clone()),
        "a monomorphic Display body is not an empty generic instance"
    );
    assert_eq!(
        entry_display_callable_key(&hew_types::EntryDisplayTarget {
            declaration: display.clone(),
            instance: hew_types::EntryCallableInstance::Generic {
                type_args: vec![ResolvedTy::I64],
            },
        }),
        MirCallableKey::instance(display, vec![ResolvedTy::I64]),
        "a generic Display body retains its concrete specialization"
    );
}
...
fn build_harness_with_target_data<'ctx>(
    ...
    CompositeHelperHarness {
        ...
        fn_symbols_by_declaration: HashMap::new(),
        collection_method_dispatches: HashMap::new(),
        frame_cleanup_thunks: RefCell::new(FrameCleanupThunkCache::default()),
        machine_step_symbols: HashSet::new(),
        actor_method_symbols: HashSet::new(),
...
fn make_test_fn_ctx<'a, 'ctx>(
    ...
    FnCtx {
        ...
        fn_symbols_by_declaration: &harness.fn_symbols_by_declaration,
        collection_method_dispatches: &harness.collection_method_dispatches,
        frame_cleanup_thunks: &harness.frame_cleanup_thunks,
        machine_step_symbols: &harness.machine_step_symbols,
        actor_method_symbols: &harness.actor_method_symbols,
