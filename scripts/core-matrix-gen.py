...
ROWS.append(
    Row(
        ...
        owns_resource=(
            TOK,
            lambda n: f"var {n}: Vec<Tok> = Vec.new();\n{n}.push(Tok {{ id: 1 }});",
            ["close 1"],
        ),
    )
)
...
ROWS.append(
    Row(
        ...
        "HashMap<string, i64>",
        mk=lambda n, i: (
            f"var {n}: HashMap<string, i64> = HashMap.new();\n"
            f'{n}.insert("a", {1 + i});\n'
            f'{n}.insert("b", {2 + i});'
        ),
        show=lambda n: (
        ...
        owns_resource=(
            TOK,
            lambda n: (
                f'var {n}: HashMap<string, Tok> = HashMap.new();\n{n}.insert("k", Tok {{ id: 1 }});'
            ),
            ["close 1"],
        ),
    )
...
ROWS.append(
    Row(
        ...
        "HashSet<i64>",
        mk=lambda n, i: (
            f"var {n}: HashSet<i64> = HashSet.new();\n"
            f"{n}.insert({1 + i});\n"
            f"{n}.insert({2 + i});"
        ),
        show=lambda n: (
...
def col_store_map_value(r):
    body = (
        r.mk("x", 0)
        + f'\nvar m: HashMap<string, {r.ty}> = HashMap.new();\nm.insert("k", x);\n'
        + 'match m.get("k") {\n'
        + "    Some(e) => {\n"
        + ind(r.show("e"), 8)
        + "\n    },\n"
...
def col_map_key(r):
    ...
    body = (
        r.mk("k", 0)
        + f"\nvar m: HashMap<{r.ty}, i64> = HashMap.new();\nm.insert(k, 9);\n"
        + "match m.get(k) {\n"
