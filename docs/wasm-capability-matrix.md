...
| `main-context-panic-cleanup` | Main-context `panic()` cleanup (`#[resource]` close, drop obligations) | Main-context panic cleanup | WASM-TODO (not checker-gated) | — | A `#[resource]` value live when `main()` panics on wasm32-wasip1 does not get its close run: `hew_panic` exits the module directly instead of unwinding through the MIR-authored landing pads that discharge drop obligations and dispatch `#[resource]` closes on native, because the shipped wasm32-wasip1 sysroot is panic=abort | WASM-TODO(main-context-panic-cleanup): |
