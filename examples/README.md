...
- **v05/surfaces/** -- One-file idiomatic demos for the landed v0.5 surfaces. Every source must ship a paired `.expected` and is gated by `make test-surface-examples`; missing or orphan expectations, nonzero exits, timeouts, and exact combined-output drift all fail the closed-corpus runner:
  ...
  - [`scanner_tokens.hew`](v05/surfaces/scanner_tokens.hew) -- line and word tokenisation through the value-state `std::io::scanner` API. It is admitted to `make test-surface-examples` with an exact expectation, so output, diagnostics, status, and timeout drift all fail the gate. Its current expectation includes the compiler's obligation under-release advisory for the loop-carried `Scanner` local; the associated compiler-side leak remains tracked separately from this behavioral authority.
