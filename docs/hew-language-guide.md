...
The native path at this checkpoint supports send/receive suspension. Nested
channel halves, `try_recv`, and heterogeneous `select` registration remain
integration work; do not infer support from a parsed arm. The intended arm is
`value from rx.recv() => ...`, without `await` in its source. See the
