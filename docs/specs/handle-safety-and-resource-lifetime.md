...
| FM-9 | WASM parity gap (native target gets the cleanup, WASM target does not) | `make playground-wasi-check` per per-handle PR | LESSONS `native-wasm-parity`; either parity green or named `WASM-TODO` (§10). |
