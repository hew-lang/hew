//! Global configuration for the Hew package manager (`~/.hew/config.toml`).
...
/// Default values applied during `hew init`.
#[derive(Debug, Default, Deserialize, Serialize)]
pub struct Defaults {
...
pub struct RegistryConfig {
    /// Override the global registry path (default `~/.hew/packages`).
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Parse { path, source } => write!(
                f,
                "invalid config '{}': expected a valid TOML Hew package-manager config, found {source}",
                path.display()
            ),
        }
    }
...
#[must_use]
pub fn get_named_registry(config: &PkgConfig, name: &str) -> Option<RemoteRegistry> {
    config
        .registries
        .as_ref()
...
/// Load the global configuration from `~/.hew/config.toml`.
///
/// Returns a default (empty) config if the file does not exist.
///
/// # Errors
///
/// Returns [`ConfigError`] when `~/.hew/config.toml` exists but cannot be read
/// or parsed as a valid Hew package-manager TOML config.
pub fn load_config() -> Result<PkgConfig, ConfigError> {
    load_config_from_path(&config_path())
}

fn load_config_from_path(path: &Path) -> Result<PkgConfig, ConfigError> {
    match std::fs::read_to_string(path) {
        Ok(text) => toml::from_str(&text).map_err(|source| ConfigError::Parse {
            path: path.to_path_buf(),
            source,
        }),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(PkgConfig::default()),
...
///
/// Falls back to `$HOME/.hew/packages` when no override is configured.
#[must_use]
pub fn registry_path(config: &PkgConfig) -> PathBuf {
...
/// Return the path to `~/.hew/config.toml`.
fn config_path() -> PathBuf {
    crate::paths::hew_home().join("config.toml")
}

/// Return the default registry path (`$HOME/.hew/packages`).
fn default_registry_path() -> PathBuf {
    crate::paths::hew_home().join("packages")
}

/// Return the path to the local package index cache (`$HOME/.hew/index/`).
#[must_use]
pub fn local_index_path() -> PathBuf {
    crate::paths::hew_home().join("index")
}
...
mod tests {
    ...
    fn load_config_reports_parse_error_with_path() {
        ...
        assert!(message.contains("expected a valid TOML Hew package-manager config"));
    }
...
"#;
        let config: PkgConfig = toml::from_str(toml_str).unwrap();
    ...
    fn parse_empty_config() {
        let config: PkgConfig = toml::from_str("").unwrap();
        assert!(config.defaults.is_none());
        assert!(config.registry.is_none());
    }
    ...
    fn registry_path_falls_back_to_default() {
        ...
        assert!(path.ends_with(PathBuf::from(".hew").join("packages")));
    }
