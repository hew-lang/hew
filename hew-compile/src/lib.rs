...
fn resolve_file_imports_internal(
    ...
    // Suppress cwd candidates when cwd is in a different Hew checkout root than
    // the source file. In that case the Tier-2 logic (build_module_search_paths_for)
    // already resolves std/ from the source file's own root; the cwd root would add
    // a second distinct std/ and trigger the ambiguity check spuriously.
    //
    ...
    let cwd_crosses_root = matches!(
        (&source_hew_root, &cwd_hew_root),
        (Some(sr), Some(cr)) if sr != cr
    );
