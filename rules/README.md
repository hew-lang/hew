...
| `unwinding-call-in-plain-c-export` (`error`) | 0    | A `#[no_mangle] extern "C"` export in `hew-runtime`/`hew-std`/`hew-cabi` calling `hew_panic`, `hew_panic_msg`, `hew_fault_trap`, `hew_trap_with_code` or a `*_walk` release walker. Those are `extern "C-unwind"`; the export must be too. A `HewValueDropThunk` reached through a layout's `drop_fn` is the same hazard and is not matched: the runtime binds it to a local whose name it shares with plain `"C"` callbacks. |
