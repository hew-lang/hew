...
fn needless_binding(ctx: &LintCtx, scrutinee_span: &Span, arms: &[MatchArm]) -> Option<String> {
    ...
    for arm in arms {
        if let Some(pattern) = some_payload(&arm.pattern.0) {
            some_binding = Some(binding_name(pattern)?);
        } else if is_none_pattern(&arm.pattern.0) {
...
/// The single sub-pattern of a `.Some(_)` arm, or `None` when `pattern` is not
/// one. `.Some(x)` is the only spelling the checker accepts since v0.6.0; the
/// bare `Some(x)` constructor spelling is refused with
/// `E_BARE_VARIANT_PATTERN`, so it is not matched here.
fn some_payload(pattern: &Pattern) -> Option<&Pattern> {
    ...
    match variant.payload.as_ref()? {
        NominalPatternPayload::Tuple(patterns) if patterns.len() == 1 => Some(&patterns[0].0),
        _ => None,
    }
}

/// Whether `pattern` denotes the `Option::None` variant, written `.None`. The
/// payload-less contextual spelling carries no parentheses, so its payload is
/// absent rather than an empty tuple.
