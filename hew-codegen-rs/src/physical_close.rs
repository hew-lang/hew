    ...
    pub(super) fn visit_close(
        ...
        match action {
            DestroyAction::Resource(id) => {
                // Only generators publish a cooperative child here. Other
                // resources release synchronously through their own authority.
                if self.module.resources[id.0 as usize].release
                    != hew_mir::physical::ResourceRelease::Generator
                {
                    return Ok(());
                }
                let handle = self
