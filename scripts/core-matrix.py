...
def write_matrix(results):
    with open(MATRIX, "w") as f:
        ...
        for name in sorted(results):
            ...
            # The detail is a human note, not a comparison key: keep it
            # host-independent (repo-relative paths) and free of trailing
            # whitespace so recordings do not churn across worktrees and
            # `git diff --check` stays clean.
            detail = detail.replace(ROOT + os.sep, "").rstrip()
            line = f"{name}\t{klass}\t{detail}\n" if detail else f"{name}\t{klass}\n"
            f.write(line)
