...
// ─── W5.007a fix: composite-nested abstract operands lower correctly ───────
//
// Regression coverage for the A622/DI-019 defect where `substitute_ty` left a
// `TypeParam` nested under a `Borrow` in the legacy `Named` shape. Driven
// end-to-end through the real pipeline so the bucket body is asserted to carry
// the single authoritative `ResolvedTy::TypeParam` shape under a borrow.

/// `fn borrow_arg<T>(x: &T) -> i64` lowers its abstract origin with the
/// borrowed parameter as `Borrow { pointee: TypeParam }`, NOT
/// `Borrow { pointee: Named }`. The borrow is confined to the parameter
/// position (no return-escape) so the body type-checks.
#[test]
fn generic_origin_with_borrow_param_lowers_to_borrow_of_type_param() {
    let pipeline = pipeline(
        r"
        fn borrow_arg<T>(x: &T) -> i64 {
            return 0;
        }
    ...
    assert_substrate_clean(&pipeline, &["borrow_arg"]);

    let poly = pipeline
        .polymorphic_mir
        .iter()
        .find(|f| f.raw.name == "borrow_arg")
        .expect("generic origin `borrow_arg` must appear in polymorphic_mir");

    let borrow_of_param = ResolvedTy::Borrow {
    ...
    assert_eq!(
        poly.raw.params,
        vec![borrow_of_param],
        "borrowed abstract parameter must be Borrow {{ TypeParam }}, got: {:?}",
        poly.raw.params
    );
    // The legacy `Named` shape for the abstract parameter must be gone: no
    // bare `Named { name: "T", args: [] }` (nor a borrow of one) anywhere in
    // the abstract signature.
    let is_legacy_named = |t: &ResolvedTy| matches!(t, ResolvedTy::Named { name, args, .. } if name == "T" && args.is_empty());
    assert!(
        !poly
            .raw
            .params
            .iter()
            .chain(std::iter::once(&poly.raw.return_ty))
            .chain(poly.raw.locals.iter())
            .any(|t| is_legacy_named(t)
                || matches!(t, ResolvedTy::Borrow { pointee } if is_legacy_named(pointee))),
        "abstract origin must not carry the legacy Named shape for `T`; got params={:?} ret={:?} locals={:?}",
        poly.raw.params,
        poly.raw.return_ty,
        poly.raw.locals,
    );
