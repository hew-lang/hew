...
if [[ ! -x "${HEW}" ]]; then
    ...
    echo "       run 'make test-vertical-slice' or set HEW_BIN to a provisioned compiler" >&2
    exit 1
fi
if [[ ! -f "${cargo_debug_dir}/libhew.a" && ! -f "${cargo_debug_dir}/hew.lib" ]]; then
    echo "error: native Hew library not found under ${cargo_debug_dir}" >&2
    echo "       run 'make test-vertical-slice' to provision the complete product" >&2
    exit 1
fi
...
# An unconsumed `#[linear]` binding is still rejected — the must-consume wall
# does not regress now that the consuming call dispatches.
if "${HEW}" compile \
...
#      dropped entirely — the dump showed neither field).
#   2. Flood a 20ms-handler actor with 20 fire-and-forget sends well past its
#      declared capacity of 4; a boolean PASS/FAIL (0/1) rather than an exact
...
# Assert a receive-gen fault fixture against its exact disposition: the
# stream-fault diagnostic, the designed trap, and the exact values yielded
# before the fault — never an allocator abort and never a silent clean EOF.
...
# the language unwind abandons its coro frame mid-execution. Crash reclamation
# owns that frame from the abandoned activation record through to its single
# free; the pump's generated drop edge must not re-enter its cleanup outline
# (#2865). The stream fault is reported after the one value yielded before the
# trap.
run_accept_capture_status "receive_gen_fn_fault_trap"
assert_receive_gen_stream_faulted "receive_gen_fn_fault_trap" "${last_accept_status}" 1
...
# Reject: cross-module direct construction — a consumer importing an `#[opaque]`
# type cannot construct it directly; only the declaring module (the producer) may
...
# Accept: nested record-let destructuring resolves a qualified imported field
# type from the same bare registry key.
run_check_run_expect_stdout "let_record_destructure_nested_selective_import"
...
#
# The exact exit codes pinned below (134 SIGABRT from glibc's `free(): invalid
# pointer`, 139 SIGSEGV) are this corruption's signature under glibc, not a
# platform-independent contract: the existing #3124 row in
# scripts/nextest-expected-failures.tsv already excludes FreeBSD for the same
# shape, and this file's own `run_compiled_binary` carries a separate non-Linux
# branch. Pin these three only where they were characterized; the fixtures
# still compile and run everywhere, but only Linux's allocator behaviour is
# proven here.
if [[ "$(uname -s)" == "Linux" ]]; then
    ...
    # binder (#3168 under #3226).
    run_accept_expect_status "match_nested_aggregate_payload_owner" 134
    if diff -u "${ROOT}/tests/vertical-slice/accept/match_nested_aggregate_payload_owner.expected" \
        ...
        echo "match_nested_aggregate_payload_owner: #3226 changed from the exact stdout-then-abort failure" >&2
    ...
    # the record's remaining field is released once, but the composite drop then
    # reads freed memory at teardown and segfaults (#3124 under #3226).
    run_accept_expect_status "projected_tuple_element_owner" 139
    if diff -u "${ROOT}/tests/vertical-slice/accept/projected_tuple_element_owner.expected" \
        "${stdout_output}" >/dev/null; then
        echo "KNOWN projected_tuple_element_owner (#3226: tuple-element projection segfaults at teardown)"
    else
        echo "projected_tuple_element_owner: #3226 changed from the exact stdout-then-segfault failure" >&2
    ...
    # garbage i64, not a crash) — only the six deterministic lines and the
    # terminal abort are pinned here (#3156 under #3226).
    run_accept_expect_status "match_param_field_payload_carrier" 134
