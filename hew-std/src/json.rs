...
/// overwrite [`hew_json_last_error`] so callers never observe a stale error from
...
/// `val` must be a valid pointer to a [`HewJsonValue`], or null.
#[no_mangle]
pub unsafe extern "C" fn hew_json_get_bytes(val: *const HewJsonValue) -> BytesTriple {
    if val.is_null() {
    ...
    if let Some(s) = v.inner.as_str() {
        ...
        let Ok(len) = u32::try_from(decoded.len()) else {
            set_parse_last_error(format!(
                "invalid JSON bytes: decoded payload of {} bytes exceeds the maximum bytes length",
                decoded.len()
            ));
            return BytesTriple {
                ptr: std::ptr::null_mut(),
