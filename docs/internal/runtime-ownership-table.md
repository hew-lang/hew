...
Status: generated inventory over main `54e8dde2c` (2026-09-01), revision 5 (revision 4 with the round-5 review corrections applied; the diff is section 8).

**What this document is, and what the compiler can read.** It is the ownership inventory of every `hew_*` C-ABI symbol, and the place where a row is *decided*. It is NOT a compiler input: nothing projects section 9 into a crate. The only FFI ownership fact the compiler can read today is `scripts/jit-symbol-classification.toml`, which `hew-types/build.rs:52` -> `generate_ffi_ownership_table` (`:60-118`) projects into `hew_types::ffi_contracts::FFI_OWNERSHIP_CONTRACTS`, and which `docs/ir-ladder.md` §6.4 names as the row HIR→SIR reads. **That rule is not enforced by any code on main, and the ladder tags it [P3]** (`docs/ir-ladder.md:2186-2191`: "**A missing row is a build error** [P3]: `ExternOwnershipFact::Absent` is rejected at HIR lowering ... today 'an absent row is deliberately not a borrow' is enforced only for resource params"), so read the admission rule below as "ready to be written", never as "the build stops without it" — section 2's admission paragraph carries the consequence. So the ladder keeps ONE compiler-readable authority — the TOML — and this table is the mint queue that feeds it: **an admitted row here is a row that is ready to be written into `scripts/jit-symbol-classification.toml`, not a row the lowering can already read.** A `br` row therefore costs one TOML edit before P1 can emit its symbol, and section 8's backlog is ordered by the phase that needs the edit. Three things the TOML schema cannot yet spell, so P1 must land them in `scripts/jit-symbol-classification.toml` + `hew-types/build.rs` alongside the rows: the `copy_value` target per carrier (`k`, section 2a), the mutate-in-place receiver (`m`, B4) and the argument binding a `rel`/`k` symbol is called with when the carrier is not a single pointer (section 2a's binding column). A fourth P1 item is not a schema change but a lowering one, and it is an open architect decision rather than a settled obligation: either the fresh emitter REFUSES an `ExternOwnershipFact::Absent` for the runtime-protocol symbols it emits, or P1 and P2 ship with those edges unguarded — nothing on main refuses one, and the ladder schedules the refusal at [P3] for `std/` declarations only (section 2, admission rule; section 8's P1 lowering row).
...
**A4 is half of a file, and the other half stays. Say which half, or P1 builds its drop glue from a table this document declares dead.** `docs/ir-ladder.md:1540-1542` says "MIR fills every `RuntimeSymbol` from `hew-mir/src/runtime_symbols.rs` — the one runtime symbol table, kept (§9)" and `:2477-2480` keeps the file without qualification, while this table lists A4 = `callee_ownership_contract` at `hew-mir/src/runtime_symbols.rs:411` among the authorities section 9 replaces. Both are right about different things and neither says so. **Decision:** `runtime_symbols.rs` keeps the SPELLINGS — the `RuntimeSymbol` constants a `GlueBody::Leaf` names — and nothing else; `callee_ownership_contract` and its `CalleeOwnershipContract {receiver, string_args, result}` verdicts are A4 and are deleted with the legacy lowerer (B9). Concretely: **`GlueDecl::Leaf.release` is the `rel` of the symbol's row in `scripts/jit-symbol-classification.toml`, called with that row's binding (section 2a); `runtime_symbols.rs` supplies the spelling of the symbol, never its ownership.** The ladder's §9 kept-list needs the same split written into it — that is an architect edit to `docs/ir-ladder.md`, not to this file.
...
| A4 | `callee_ownership_contract` | `hew-mir/src/runtime_symbols.rs:411` string-keyed `CalleeOwnershipContract {receiver, string_args, result}` | receiver/string-arg/result ownership by callee spelling | 155 spellings |
...
**The ladder names the other symbol, and one of the two documents has to move before P2 starts.** `docs/ir-ladder.md:1574-1577` §5.2 item 6 spells the Vec leaf release as "`Vec<T>` -> `hew_vec_free_owned` (`hew_vec_free` for BitCopy elements, vec.rs:1593/2953)", which is the split this bug resolves away. Both symbols are live admitted `br` rows in the fence today (`{n="hew_vec_free",d="vec:1593",p="c",r="N",e="br"}` and `{n="hew_vec_free_owned",d="vec:2953",p="c",r="N",e="br",f=["B4"]}`) and both bodies are `unsafe { free_vec_descriptor(v) }`, so either spelling links and runs; the disagreement is invisible at run time and only shows up at P5 when one of them is deleted. A P2 lane building `GlueBody::Leaf` from the ladder emits a symbol P5 removes; a lane building it from this resolution emits a symbol the ladder does not name. **Recommendation:** ladder §5.2 item 6 changes to `hew_vec_free` for every element class, with `hew_vec_free_owned` named only as the constructor-provenance alias `VO`. That is an architect edit to `docs/ir-ladder.md`; this table's resolution does not move.
...
| P1 (lowering) | the fresh lowering must REFUSE `ExternOwnershipFact::Absent` for the runtime-protocol symbols it emits; on main `Absent` is a value that falls through to A3/A4 defaults (`lower/facts.rs:1295-1302`, `runtime_call.rs:2025-2040`) and the ladder schedules the rejection at [P3] for `std/` declarations only | open — architect decision (section 2, admission rule): move the refusal into P1 for the emitter's own callees, or state that P1 and P2 ship unguarded |
| P2 (collections) | `hew_hashmap_free_layout`, `hew_hashset_free_layout`, `hew_hashmap_clone_layout`, `hew_hashset_clone_layout` (`br-`); the three TOML rows in B15 (`hew_vec_get_clone`, `hew_vec_take_owned`, `hew_vec_get_str`); **introduced**: `hew_hashmap_insert_layout_move`, `hew_hashset_insert_layout_move` (definition + `rr=True` entry + TOML row in the same change); **cross-doc**: ladder §5.2 item 6 must name `hew_vec_free`, not `hew_vec_free_owned` (B4) | open |
...
Nothing in revision 5 is a re-read that promotes a row; the four architect decisions it raises instead — where the `Absent` refusal lands (P1 or P3), which symbol the ladder's Vec leaf release names, how the ladder's §9 kept-list splits `runtime_symbols.rs`, and where the full profile is committed — are the ones a P1 or P2 brief cannot be written without.
