...
# The full runner records a `compile_accept` failure instead of exiting the
# whole run, and every caller stops its own fixture on that failure rather
# than continuing to run further checks against a binary that doesn't exist.
# shellcheck disable=SC2016  # Match the literal expansion syntax in run.sh.
grep -qF 'record_failure "${fixture}" "compile-accept.sh failed' "${RUNNER}"
# shellcheck disable=SC2016  # Match the literal expansion syntax in run.sh.
grep -qF 'compile_accept "${fixture}" || return 0' "${RUNNER}"

# The failure accumulator records every unexpected outcome instead of
# stopping at the first one: two independent failures both survive to the
# closing summary. Exercise the actual `record_failure` from run.sh (not a
# reimplementation) by extracting and evaluating just that function.
accumulator_src="$(sed -n '/^record_failure() {/,/^}/p' "${RUNNER}")"
if [[ -z "${accumulator_src}" ]]; then
    echo "could not find record_failure() in ${RUNNER}" >&2
    exit 1
fi
eval "${accumulator_src}"
fail_count=0
fail_names=()
fail_reasons=()
record_failure "fixture_one" "reason one" 2>/dev/null
record_failure "fixture_two" "reason two" 2>/dev/null
if [[ "${fail_count}" -ne 2 ]]; then
    echo "expected two recorded failures, got ${fail_count}" >&2
    exit 1
fi
if [[ "${fail_names[0]}" != "fixture_one" || "${fail_names[1]}" != "fixture_two" ]]; then
    echo "expected both fixture_one and fixture_two to be recorded, got: ${fail_names[*]}" >&2
    exit 1
fi
if [[ "${fail_reasons[0]}" != "reason one" || "${fail_reasons[1]}" != "reason two" ]]; then
    echo "expected both reasons to be recorded, got: ${fail_reasons[*]}" >&2
    exit 1
fi
