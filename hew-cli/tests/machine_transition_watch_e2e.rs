...
fn select_record_element_cross_block_arm_runs_clean() {
    run_inline_scribbled(
        ...
        "import std.channel.channel;\n\
         ...
         actor Combined {\n\
         \x20   receive fn run() {\n\
         \x20       let (tx, rx): (channel.Sender<Transition>, channel.Receiver<Transition>) = channel.new(4).expect(\"channel allocation failed\");\n\
...
fn cross_actor_record_transition_watch_runs_clean() {
    run_inline_scribbled(
        ...
        "import std.concurrency.lifecycle;\n\
         ...
         \n\
         fn main() {\n\
         \x20   let (tx, rx): (channel.Sender<Transition>, channel.Receiver<Transition>) = channel.new(8).expect(\"channel allocation failed\");\n\
         \x20   let (done_tx, done_rx): (channel.Sender<i64>, channel.Receiver<i64>) = channel.new(1).expect(\"channel allocation failed\");\n\
         \x20   let obs = spawn Observer;\n\
...
fn machine_snapshot_select_watch_matches_state_variants() {
    run_inline_scribbled(
        ...
        "\
         ...
         \x20       let (tx, rx): (channel.Sender<Conn>, channel.Receiver<Conn>) = channel.new(4).expect(\"channel allocation failed\");\n\
         \x20       var c: Conn = Conn.Idle;\n\
         \x20       c.step(ConnEvent.Connect);\n\
         \x20       tx.send(c);\n\
