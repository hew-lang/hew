...
fn main() {
    ...
    var next1: Vec<i64> = Vec.new(); next1.push(2);
    let updated1 = VHolder { items: next1, ..base };
    var next2: Vec<i64> = Vec.new(); next2.push(3);
...
type VHolder { items: Vec<i64>, tag: string }
fn main() {
    var init: Vec<i64> = Vec.new(); init.push(7);
    let base = VHolder { items: init, tag: "base" };
    var next: Vec<i64> = Vec.new(); next.push(8);
...
type VecHolder { items: Vec<i64>, tag: i64 }
fn main() {
    var init: Vec<i64> = Vec.new(); init.push(5);
...
import std.string;
type Inner { label: string, n: i64 }
type Outer { pair: (Inner, i64), tag: i64 }
fn main() {
...
type Inner { label: string, n: i64 }
type Mid { inner: Inner, k: i64 }
type Outer { items: Vec<Mid>, tag: i64 }
fn main() {
...
fn reject_by_value_param_base() {
    let source = r#"
import std.string;
type Cfg { name: string, k: i64 }
type Wrap { cfg: Cfg, tag: i64 }
...
fn reject_carry_closure_field() {
    let source = r#"
import std.string;
type Boxx { tag: string, cb: fn() -> string }
...
fn accept_carry_tuple_of_owned_field() {
    let source = r#"
import std.string;
type T { pair: (string, i64), tag: string }
fn mk() -> T {
...
fn accept_carry_nested_and_collection_bearing_tuples() {
    let cases: &[(&str, &str)] = &[
        (
            ...
            r#"
import std.string;
type T { pair: (string, (string, i64)), tag: string }
fn mk() -> T {
...
"#,
        ...
        (
            ...
            r#"
import std.string;
type T { pair: ((string, i64), (string, i64)), tag: string }
fn mk() -> T {
...
import std.string;
type Inner { label: string, n: i64 }
type T { pair: (Inner, i64), tag: string }
fn mk() -> T {
...
import std.string;
type Inner { label: string, n: i64 }
type T { pair: (Option<Inner>, i64), tag: string }
fn mk() -> T {
...
fn reject_carry_non_heap_tuple_field_is_conservative() {
    let source = r#"
import std.string;
type T { pair: (i64, i64), tag: string }
fn mk() -> T {
...
import std.string;
type Inner { label: string, n: i64 }
type O { maybe: Option<Inner>, tag: string }
fn mk() -> O {
...
fn reject_carry_vec_closure_field_clean_no_panic() {
    let source = r#"
import std.string;
type P { keep: Vec<fn() -> string>, churn: string }
fn main() {
    var v: Vec<fn() -> string> = Vec.new();
...
fn carry_rule_matrix_cases() -> Vec<CarryRuleMatrixCase> {
    vec![
        ...
        CarryRuleMatrixCase {
            ...
            depth: 2,
            ty: "Wrapper",
            declarations: "type Wrapper { inner: (string, i64), tag: i64 }",
            setup: "",
        ...
        CarryRuleMatrixCase {
            ...
            depth: 3,
            ty: "Wrapper",
            declarations: "type Wrapper { inner: ((string, i64), i64), tag: i64 }",
            setup: "",
        ...
        CarryRuleMatrixCase {
            ...
            depth: 4,
            ty: "Wrapper",
            declarations: "type Wrapper { inner: (((string, i64), i64), i64), tag: i64 }",
            setup: "",
        ...
        CarryRuleMatrixCase {
            ...
            ty: "Vec<Leaf>",
            declarations: "",
            setup: "var leaves: Vec<Leaf> = Vec.new(); leaves.push(Leaf { label: string.repeat(\"k\", 32), n: 1 });",
            value: "leaves",
            accepted: true,
        },
        CarryRuleMatrixCase {
