...
unsafe fn prepare_quiescent_actor_for_cleanup(actor: *mut HewActor) {
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        // delivered to a freed actor. Keyed by the actor's INCARNATION, matching
        // the identity the reactor captured at registration time: an address
        // would also name whichever actor next inherits this box.
        // SAFETY: the caller guarantees `actor` is valid and quiescent here.
