...
fn checked_calls_and_task_values_compose_in_expression_positions() {
    for source in [
        ...
        "actor Worker { receive fn process(value: i64) -> i64 { value + 1 } } fn main() { let worker = spawn Worker; let task = fork worker.process(41); let reply = await task; let _ = reply; }",
        "actor Worker { receive fn process() -> Result<i64, string> { .Ok(42) } } fn main() { let worker = spawn Worker; let task = fork worker.process(); let reply = await task; let _ = reply; }",
