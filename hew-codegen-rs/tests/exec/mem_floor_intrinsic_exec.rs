//! WASM-parity coverage for the memory-intrinsic floor (W5.005 / F1b,
//! Decision 4 Option A).
//!
...
//! target-parity half: a full driver pipeline that threads a pointer from
//! `mem.alloc` through `mem.ptr_offset`/`mem.ptr_copy` into `mem.dealloc`
//! must emit for wasm32 exactly as it does natively — the floor is not a
//! native-only substrate and its wasm module links.
//!
//! There is NO user surface for these primitives (A605 gates them to the
//! `std.mem` floor) and no container consumer yet, so the pipeline is
//! hand-built rather than lowered from Hew source.
