...
require_value() {
    [[ $# -gt 1 ]] || { echo "error: $1 requires a path" >&2; exit 1; }
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --hew-bin)
            ...
            require_value "$@"; HEW_BIN_ARG="$2"; shift 2 ;;
        --emit-o0-outcomes)
            reject_unless_corpus --emit-o0-outcomes hew-suite
            require_value "$@"; EMIT_O0_OUTCOMES_FILE="$2"; shift 2 ;;
        --junit-output)
            reject_unless_corpus --junit-output hew-suite
            require_value "$@"; JUNIT_OUTPUT_ARG="$2"; shift 2 ;;
        --outdir)
            reject_unless_corpus --outdir doc-fences
            require_value "$@"; OUTDIR_ARG="$2"; shift 2 ;;
        --help|-h)
            usage; exit 0 ;;
        *)
...
# ── Corpus: hew-suite ─────────────────────────────────────────────────────────
#
# `hew test` is run per fixture file and each JUnit report is cached under a key
# covering the compiler content and the fixture content, so a rerun with an
# unchanged compiler compiles nothing. The reports are merged and parsed by
# scripts/lib/hew_junit.py rather than regex-matched out of the human-readable
# text: a cosmetic format change can no longer break pass/fail extraction, and
# the merged report is the one a CI job uploads to the checks UI.

STDERR_FILE=""

run_hew_suite() {
    local tests_dir junit_output cache_dir hew_junit_py
    local fixture fixture_hash compiler_hash cache_key cached_report fresh_report rc
    local parsed summary_line report_total report_failures report_skipped
    local reports=()
    local status name parsed_failed
    ...
    for fixture in "$tests_dir"/*.hew; do
        ...
        if [[ ! -s "$cached_report" ]] || ! python3 "$hew_junit_py" "$cached_report" >/dev/null 2>&1; then
            ...
            # per file must too, or caching turns them into failures. A clean
            # exit with no report means no tests, not a broken fixture.
    ...
    # hew_junit.py prints one "<status>\t<name>" line per test plus a trailing
    # "__SUMMARY__\t<total>\t<failures>\t<skipped>" line, and exits non-zero on
    # malformed XML or a <testsuites> whose failures attribute disagrees with
    # its own <failure> element count.
    parsed=""
    ...
    # ratchet verdict is known — the outcome set is valid either way.
