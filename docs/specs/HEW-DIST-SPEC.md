...
- distributed node identity and actor-reference shape
...
| ActorRef | A reference to an actor endpoint; identity includes node and incarnation data. |
...
`ActorRef` identity is defined as `(NodeId, slot, incarnation)`:
...
1. generation-checked `ActorRef` + node incarnation — gated by sections 3 and 5
