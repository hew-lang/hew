...
fn assert_sample_tail_call_resolves(
    source: &str,
    expected: &str,
    expected_intrinsic: MathIntrinsic,
) {
    ...
    let main_fn = output
        ...
        .find_map(|item| match item {
            HirItem::Function(function) if function.name == "sample" => Some(function),
            _ => None,
        })
        .expect("sample function must be present");
    let tail = main_fn.body.tail.as_deref().expect("sample must have tail");
    let HirExprKind::Call { callee, .. } = &tail.kind else {
        panic!("sample tail must be a call, got {tail:#?}");
    };
...
fn math_builtin_identifiers_resolve_to_typed_intrinsics() {
    for (name, intrinsic, source) in [
        (
            "sqrt",
            MathIntrinsic::Sqrt,
            "fn sample() -> f64 { math.sqrt(9.0) }",
        ),
        (
            "exp",
            MathIntrinsic::Exp,
            "fn sample() -> f64 { math.exp(1.0) }",
        ),
        (
            "log",
            MathIntrinsic::Log,
            "fn sample() -> f64 { math.log(1.0) }",
        ),
        (
            "sin",
            MathIntrinsic::Sin,
            "fn sample() -> f64 { math.sin(1.0) }",
        ),
        (
            "cos",
            MathIntrinsic::Cos,
            "fn sample() -> f64 { math.cos(1.0) }",
        ),
        (
            "abs",
            MathIntrinsic::AbsI64,
            "fn sample() -> i64 { math.abs(-9) }",
        ),
        (
            "min",
            MathIntrinsic::MinI64,
            "fn sample() -> i64 { math.min(7, 3) }",
        ),
        (
            "max",
            MathIntrinsic::MaxI64,
            "fn sample() -> i64 { math.max(7, 3) }",
        ),
        (
            "abs_f",
            MathIntrinsic::AbsF64,
            "fn sample() -> f64 { math.abs(-9.0) }",
        ),
        (
            "min_f",
            MathIntrinsic::MinF64,
            "fn sample() -> f64 { math.min(7.0, 3.0) }",
        ),
        (
            "max_f",
            MathIntrinsic::MaxF64,
            "fn sample() -> f64 { math.max(7.0, 3.0) }",
        ),
        (
            "pow",
            MathIntrinsic::Pow,
            "fn sample() -> f64 { math.pow(2.0, 10.0) }",
        ),
        (
            "floor",
            MathIntrinsic::Floor,
            "fn sample() -> f64 { math.floor(2.75) }",
        ),
        (
            "ceil",
            MathIntrinsic::Ceil,
            "fn sample() -> f64 { math.ceil(2.25) }",
        ),
        (
            "round",
            MathIntrinsic::Round,
            "fn sample() -> f64 { math.round(2.5) }",
        ),
    ] {
        assert_sample_tail_call_resolves(source, name, intrinsic);
    }
}
...
fn math_intrinsic_module_alias_retains_canonical_typed_target() {
    assert_sample_tail_call_resolves(
        "import std.math as trig; fn sample() -> f64 { trig.sqrt(9.0) }",
        "sqrt",
        MathIntrinsic::Sqrt,
    );
}
