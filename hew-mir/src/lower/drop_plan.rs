...
pub(super) fn elaborate(
    ...
    let mut pending_for_await_terminal_drops: HashMap<usize, Vec<(BindingId, ElabDrop)>> =
        HashMap::new();
    for handoff in builder.for_await_handle_handoffs.iter().rev() {
        ...
        for (plan_index, (exit, plan)) in drop_plans.iter().enumerate() {
            ...
            {
                pending_for_await_terminal_drops
                    .entry(plan_index)
                    .or_default()
    ...
    // Re-admit it only on abandonment exits in the hand-off-to-inline-close
    // region. Returns already emit the cursor's inline edge drop; cutting the
    // region at the raw close prevents a stale dataflow `Live` state from
    // reviving it after normal exhaustion.
    for handoff in builder.for_await_handle_handoffs.iter().rev() {
        let Some(descriptor) = stream_handle_drop_descriptor(&handoff.ty) else {
            continue;
        };
        ...
        let abandonment_region = for_await_cursor_abandonment_region(
            &checked.blocks,
    ...
    for (plan_index, mut pending) in pending_for_await_terminal_drops {
        pending.sort_by_key(|(binding, _drop)| {
            std::cmp::Reverse(*declaration_rank.get(binding).unwrap_or(&usize::MAX))
        });
        let plan = &mut drop_plans[plan_index].1;
        pending.retain(|(_binding, drop)| {
            !plan
                .drops
                .iter()
...
fn for_await_cursor_abandonment_region(
    blocks: &[BasicBlock],
    handoff_block: u32,
    cursor_place: Place,
