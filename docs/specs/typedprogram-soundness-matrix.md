...
2. `method_call_receiver_kinds` now has a checker-side post-pass validator (`validate_method_call_receiver_kinds_output_contract`) that removes entries referencing unknown type or trait names; graduated to full boundary-certified status in wave13-typedprogram-boundary-cert-2.
...
2. ~~Add dedicated negative tests for erased `assign_target_kinds`, `assign_target_shapes`, and `lowering_facts` entries.~~ **Done** in wave13-typedprogram-boundary-cert-2.
3. ~~Decide whether `method_call_receiver_kinds` needs a checker-side output-boundary validator analogous to `validate_expr_output_contract` / `validate_assign_target_output_contract`.~~ **Done** in wave13-typedprogram-boundary-cert-2 — `validate_method_call_receiver_kinds_output_contract` added.
