...
| `ownership-over-locks` | runtime/concurrency | Shared mutable state crosses threads, actors, or background workers. | Fix ownership and causality first; locks cannot rescue a broken lifetime model. | Prefer `Arc`, atomics, owned routing tables, and mailbox-carried context, never pass stack pointers across threads, then add the smallest lock surface still required. | Journey audit runtime findings; PR #784; original LESSONS ownership lesson. |
