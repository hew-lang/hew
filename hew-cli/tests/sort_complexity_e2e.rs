...
fn check_strings() -> i64 {
    var duplicate_reverse: Vec<string> = Vec.new();
    for i in 0 .. 256 {
    ...
    var non_power: Vec<string> = Vec.new();
    for i in 0 .. 17 {
        let value = match i % 3 {
            0 => "charlie",
            1 => "alpha",
