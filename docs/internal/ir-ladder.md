...
| `vec_authority::is_copy_layout`; `ValueClass::Unknown` and its 15 sites; `hew-hir/src/value_class.rs` (copied, not moved); `BuiltinTypeMarker::Resource` on `LocalPid`/`HewActor`/`BoxedActor` and `None` on the BitCopy scalars/enums of §1.1 (markers corrected). Each is a reader or an input of the legacy lowering path, which is the parity oracle until P5, so none can move while that leg must agree with itself: flipping `LocalPid` routes a `Vec<LocalPid<_>>` element off its pointer ABI, and flipping `HewActor` breaks the close-method registration every builtin class-table seed runs. Until then the disagreement is recorded rather than resolved, by `builtin_marker_and_the_class_table_agree`. | `vec_authority.rs:270`, `hew-hir`, `hew-types/src/builtin_type.rs:355` | P5 |
...
| `dataflow.rs` (owner replay / move checker over `MirStatement`), `return_provenance.rs` (7440) + `return_provenance_ref.rs`, `ownership.rs` (2183, `ValueOwnership`/`CowHeapRelease`), `thunk_requirements.rs` resource lookups by name, `model.rs` `ty_drop_obligation_inner`/`ty_carries_drop_obligation*`/`ty_owns_heap_mir`/`container_ingress_is_copy_in`/`CloseObligationRegistry` string join (1834), `MirStatement` stream | `hew-mir/src` | P5 |
