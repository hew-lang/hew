...
| `g` | glue | thunk / descriptor pointer (`drop_fn`, `clone_fn`, `HewValueLayout`, `HewMapKeyLayout`, ...) |
...
| --- | --- | --- | --- | --- |
| A1 | `[[ownership.contracts]]` | `scripts/jit-symbol-classification.toml`, projected by `hew-types/build.rs` into `ffi_contracts::FFI_OWNERSHIP_CONTRACTS` | per-symbol `result`/`params`/`release-symbol`/`discharge-depth`/`result-retention`; consumers are the checker's source-declared `extern "rt"` join (`hew-types/src/check/registration.rs`, `hew-hir/src/lower.rs`), `hew-sir/src/resource.rs` (nominal resource release/producer verification), and `RuntimeCallFamily::consumes_receiver` / `FileReadHandleKind::matches` (only for `TcpAttachLocal` and `FileReadOp::Open`, `hew-types/src/runtime_call.rs` and `runtime_call/file_resources.rs`). Absence is fail-closed everywhere except the two literal-symbol lookups above, which are pinned by test (D420: the drifting unclassified-row count and its ratchet were deleted, they had no consumer) | not tracked by count |
...
| --- | --- | --- | --- |
| Vec | `HewValueLayout.{clone_fn,drop_fn}` (`hew-cabi/src/value.rs`) | constructor aborts when `ownership_kind != Plain && drop_fn.is_none()` (`vec.rs:268`); `clone_fn` checked only at first clone use (`vec.rs:2571`) | keep the abort; make `clone_fn` mandatory at construction for non-Plain (`hew_vec_new_with_elem_layout`), since a release-only descriptor makes `hew_vec_clone_owned` a runtime abort the compiler cannot see |
