...
This gate closes it, in five directions:
  ...
  A5  every declared phony target has a recipe or prerequisites, so resolving a
      target name cannot silently succeed without doing work.
  A6  the committed-baseline set is closed, asked of the TREE: every tracked
      file whose name says it is a derived baseline is regenerated by exactly
      one member of `scripts/baselines.py`, or is listed there as not derived
      with a reason (A6a). A secondary pass asks the same question of the gates,
      for a comparison file whose name carries no such shape (A6b). A committed
      baseline whose only regen path is folklore is how ten of the thirty-five
...
#
# Two earlier versions asked the question from the GATE side and both could be
# walked past. Matching target names missed `test-ux-examples`, which compares
# every tutorial against a committed `.expected` and is named nothing of the
# kind. Analysing what each gate's scripts read missed anything reached through
# a variable — `tests/mir-baselines/funcupdate-reassign` and
# `hew-cli/tests/fixtures/release-count-baseline.tsv` are both named only inside
# Rust test code, and both turned out to be real baselines with real regen
# commands that nothing was holding to the registry.
#
# The authority is therefore the FILE closure (`baselines.coverage`), in
# scripts/baselines.py: every tracked file matching a baseline SHAPE is owned by
# exactly one member or reasoned away. How a script reaches a file cannot affect
# whether the file exists. The recipe scan below is kept as a secondary net,
# because it costs nothing and catches the converse — a gate comparing against a
# committed file whose name carries no shape at all.
...
def baseline_file_findings() -> tuple[list[tuple[str, str]], list[str]]:
    """Close the baseline set over the TREE. This is the authority.

    Every tracked file whose name says it is a committed baseline must be
    covered by exactly one registry member, or be listed in NO_BASELINE_FILES
    with the reason it is not derived. Asking the question of the files rather
    than of the gates is what makes the answer unbypassable: it does not matter
    which script reads the file, how it builds the path, or what the gate is
    called. Returns (findings, patterns that no longer match anything).
    """
    baselines = _baselines_module()
    findings, stale = baselines.coverage(baselines.tracked_files())
    detailed = [
        (
            path,
            (
                f"this tracked file is claimed by {len(owners)} registry members "
                f"({', '.join(owners)}); two regen commands writing the same bytes "
                "means neither owns it. Narrow one member's paths."
            )
            if owners
            else (
                "this tracked file is shaped like a committed baseline and no "
                "registry member regenerates it. Add it to scripts/baselines.py "
                "with the command that produces it, or add it to "
                "NO_BASELINE_FILES there with the reason it is not derived."
            ),
        )
        for path, owners in findings
    ]
...
def baseline_membership_findings(
    ...
    The SECONDARY net, kept because it costs nothing and catches the converse
    case: a gate comparing against a committed file whose name carries none of
    the baseline shapes. The file closure above is the authority.
    """
    baselines = _baselines_module()
...
def main() -> int:
    ...
    # A `--no-run` invocation BUILDS test binaries and runs none of them, so it
    # cannot shrink the executed corpus — which is the whole concern here. The
    # preflight derives its warm-up commands from each gate's recipe, so those
    # build-only forms now appear in this text; reading them as "CI runs a fast
    # tier" would fail the gate on a command that runs no tests at all.
    running_ci_text = "\n".join(
        line for line in ci_text.splitlines() if "--no-run" not in line
    )
    bad_profiles = sorted(
        set(re.findall(r"--profile\s+([A-Za-z0-9_-]+)", running_ci_text))
    ...
    #
    # Authority first: the file closure over the tree. Then the secondary net
    # over gate recipes, for a comparison file whose name carries no shape.
    baselines_mod = _baselines_module()
    shaped = [
        path
        for path in baselines_mod.tracked_files()
        if path and baselines_mod.is_baseline_shaped(path)
    ]
    file_findings, stale_no_baseline = baseline_file_findings()
    print(
        f"\n==> A6a: derived-baseline file closure ({len(shaped)} baseline-shaped "
        "tracked file(s))"
    )
    for path, detail in file_findings:
        findings.fail("A6", path, detail)
    for pattern in stale_no_baseline:
        findings.fail(
            "A6",
            f"scripts/baselines.py: NO_BASELINE_FILES[{pattern!r}]",
            "this entry matches no tracked file, so it records a decision about "
            "nothing. Delete it.",
        )
    print(
        f"    {len(shaped) - len(file_findings)}/{len(shaped)} baseline-shaped files "
        f"are owned by exactly one member or reasoned; {len(stale_no_baseline)} "
        "stale no-baseline entry/entries."
    )

    unregistered, stale_exemptions = baseline_membership_findings(recipes)
    discovered = comparison_targets(recipes)
    print(
        f"\n==> A6b: gate-recipe cross-check ({len(discovered)} comparison "
    ...
    # ── A8: advisory static consumer scan ────────────────────────────────────
    #
    ...
    advisory_rows = sorted(
        (gate, path) for gate, gate_paths in likely.items() for path in gate_paths
    )
    print(
        f"\n==> A8: advisory consumer scan "
        f"({len(advisory_rows)} likely undeclared input(s))"
    )
    for gate, path in advisory_rows[:25]:
    ...
    dangling_embeds = sorted(
        (target, crates)
        for target, crates in advisory_declarations.embeds.items()
        if target not in tracked
    )
    print(
        f"\n==> A9: compiled-in assets are tracked "
        f"({len(advisory_declarations.embeds)} embedded asset(s))"
    )
    for target, crates in dangling_embeds:
        findings.fail(
            "A9",
            target,
            f"{', '.join(crates)} compiles this path in with include_str!/"
            "include_bytes!, but it is not a tracked file. Fix the include path, "
            "or track the asset.",
        )
    print(
        f"    {len(advisory_declarations.embeds) - len(dangling_embeds)}/"
