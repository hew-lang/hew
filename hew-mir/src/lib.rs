...
pub use model::{
    ...
    is_indirect_enum, machine_enum_view, machine_enum_views, machine_event_enum_view,
    machine_event_enum_views, mangle_dyn_drop_in_place_symbol, mangle_dyn_thunk_symbol,
    mangle_dyn_vtable_symbol, ty_carries_drop_obligation, ty_carries_drop_obligation_mir,
    ty_contains_closure_value, ty_contains_heap_owning, ty_contains_unclonable_opaque,
    ty_contains_unclonable_opaque_with_names, ty_drop_obligation, ty_heap_ownership, ty_owns_heap,
    ty_owns_heap_mir, validate_context_markers, ActorHandlerKind, ActorHandlerLayout, ActorLayout,
    ActorStateLoadMode, ActorStateStoreHandoff, AggregateOwner, BasicBlock, BlockKind, BorrowKind,
    CallAuthority, CaptureKind, CheckedMirFunction, ChildInitArg, CloseObligationRegistry,
    ClosureEnvAllocation, ClosureEnvFieldInit, ClosureEnvFieldOwnership, ClosureEnvMode,
    ClosurePairVecKind, CmpPred, CollectionLayoutProbeKind, CompilerCallKind, CooperateKind,
    CooperateSite, CoroutineFacts, CoroutineSchema, DecisionFact, Direction, DropFnSpec, DropKind,
    DropObligation, DropPlan, DynVtableInstance, ElabBlock, ElabDrop, ElabDropGuard,
    ElaboratedMirFunction, EnumLayout, ExitPath, ExternDecl, ExternStringOwnership, FieldAddr,
    FieldOffset, FloatWidth, FunctionCallConv, GeneratorEnvFieldPlan, GeneratorEnvPlan,
    HeapOwnership, HeapOwnershipLayouts, IdentityAggregateKind, Instr, IntArithOp, IntSignedness,
    IrPipeline, JoinBranch, LambdaActorShape, LambdaCapture, LambdaEnvFieldDrop, MachineLayout,
    MachineVariantLayout, MirCallableInstance, MirCallableKey, MirCheck, MirConst, MirConstValue,
    MirDiagnostic, MirDiagnosticKind, MirHeapLayouts, MirLint, MirScope, MirStatement,
    ModuleCapabilities, NeutralizeAuthority, OwnerId, OwnershipEvent, OwnershipGuardKind,
    ParamBoundaryFact, ParamBoundaryMode, ParamCrashCleanupKind, ParamLoanStorage,
    ParamRepresentationEffect, Place, PointerWidth, PolymorphicMirFunction, PoolCount,
    PreparedCarrierBoundary, ProjectedPayloadRejectReason, RawMirFunction, RawValueDef, RawValueId,
    RawValueOp, RecordLayout, RegexLiteral, RuntimeCall, SelectArm, SelectArmKind, SendAliasMode,
    SourceOrigin, SpawnEnvFieldOwnership, StableActorRole, Strategy, StringRetainCondition,
    SupervisorChildLayout, SupervisorConfigParam, SupervisorLayout, SuspendKind,
    SynthesizedCallable, Terminator, TraitObjectStorage, TrapKind, ValueMaterializationReason,
