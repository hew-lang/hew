...
- **Distributed node API / remote-actor messaging**: The `Node::*` cluster API
  ...
  The checker rejects the whole `Node::` namespace and remote messaging with
  `Distributed`, so distributed programs fail closed at check time the same way
  `net.*` / `quic.*` / `tls.*` do. Basic single-process actors (`spawn` /
  `send` / `ask`) remain available.
...
Rows whose **Checker disposition** is **WASM-TODO** are intentionally _not_
...
- `hew-types/src/check/methods.rs :: check_method_call` (stream._ / `http_client._`/`smtp._` / http._ / net._ / process._ / tls._ / quic._ / dns._ / os._ / `crypto.random_bytes` module calls)
