...
| `E_SEND_RESULT_DROPPED`                        | A statement-position send or ask whose typed delivery outcome is discarded (§2.1.1, §5.6). The fix-it writes `let _ = pid.m();` when the outcome is genuinely not wanted.                                                                                                         |
