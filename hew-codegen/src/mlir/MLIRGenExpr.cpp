...
// ============================================================================
// Field-yield scanner — shared between generateExprCall and generatePrintCall.
//
// Recursively checks whether an expression yields a struct field access
// through wrapper forms (block, if, if-let, match, unsafe, scope).  Used by
// the fail-closed pre-scan in both the generic call path and the print/println
// Display path so both rejection gates cover the same set of expression shapes.
//
// blockYieldsFieldAccess checks the value-position of a Block: trailing expr
// first, then (when absent) the last statement's expression, StmtIf,
// StmtMatch, or loop-family (via break values).  Mirrors
// collectExcludeVarsFromBlock.
//
// blockBreakYieldsFieldAccess scans a loop body for StmtBreak nodes whose
// break-value yields a field access, recursing through nested control flow
// (StmtIf/else-if chains, StmtIfLet, StmtMatch, StmtExpression wrappers, and
// nested loops for labeled breaks).
//
// SCOPE: the loop-break path is defense-in-depth — the type checker currently
// rejects loop break values (loops produce ()), so the codegen guard does not
// fire today.  It covers the case where typed break-values are later added.
// ============================================================================

// Forward declarations for mutual recursion.
static bool exprYieldsFieldAccess(const ast::Expr &expr);
static bool blockYieldsFieldAccess(const ast::Block &blk);
static bool stmtIfYieldsFieldAccess(const ast::StmtIf &ifStmt);
static bool blockBreakYieldsFieldAccess(const ast::Block &blk,
                                        const std::optional<std::string> &targetLabel,
                                        bool checkUnlabeled);
static bool exprBreakYieldsFieldAccess(const ast::Expr &expr,
                                       const std::optional<std::string> &targetLabel,
                                       bool checkUnlabeled);
static bool stmtIfBreakYieldsFieldAccess(const ast::StmtIf &ifStmt,
                                         const std::optional<std::string> &targetLabel,
                                         bool checkUnlabeled);
...
static bool stmtIfYieldsFieldAccess(const ast::StmtIf &ifStmt) {
  ...
  if (ifStmt.else_block) {
    if (ifStmt.else_block->block && blockYieldsFieldAccess(*ifStmt.else_block->block))
      return true;
...
static bool blockBreakYieldsFieldAccess(const ast::Block &blk,
  ...
  for (const auto &stmt : blk.stmts) {
    ...
    if (auto *innerFor = std::get_if<ast::StmtFor>(&stmt->value.kind)) {
      const auto innerTarget = (innerFor->label && targetLabel && *innerFor->label == *targetLabel)
                                   ? std::optional<std::string>{}
                                   : targetLabel;
...
// blockYieldsFieldAccess: check the value-position of a Block expression.
static bool blockYieldsFieldAccess(const ast::Block &blk) {
  if (blk.trailing_expr)
    return exprYieldsFieldAccess(blk.trailing_expr->value);
  if (!blk.stmts.empty()) {
...
// exprYieldsFieldAccess: entry point — returns true if the expression is or
// wraps a field access in value position.
static bool exprYieldsFieldAccess(const ast::Expr &expr) {
  if (std::holds_alternative<ast::ExprFieldAccess>(expr.kind))
    return true;
  if (auto *blockE = std::get_if<ast::ExprBlock>(&expr.kind))
    return blockYieldsFieldAccess(blockE->block);
  if (auto *ifE = std::get_if<ast::ExprIf>(&expr.kind)) {
  ...
  if (auto *ifLet = std::get_if<ast::ExprIfLet>(&expr.kind)) {
    if (blockYieldsFieldAccess(ifLet->body))
      return true;
    if (ifLet->else_body && blockYieldsFieldAccess(*ifLet->else_body))
      return true;
    return false;
  }
  if (auto *matchE = std::get_if<ast::ExprMatch>(&expr.kind)) {
  ...
  if (auto *unsafeE = std::get_if<ast::ExprUnsafe>(&expr.kind))
    return blockYieldsFieldAccess(unsafeE->block);
  if (auto *scopeE = std::get_if<ast::ExprScope>(&expr.kind))
    return blockYieldsFieldAccess(scopeE->block);
  return false;
}
...
mlir::Value MLIRGen::generateCallExpr(const ast::ExprCall &call, const ast::Span &exprSpan,
  ...
  if (calleeName == "hew_stream_next_bytes") {
    ...
    auto rawPtr = hew::RuntimeCallOp::create(builder, location, mlir::TypeRange{ptrType},
                                             calleeAttr, mlir::ValueRange{streamVal})
                      .getResult();
    ...
    auto optLLVMType =
        mlir::LLVM::LLVMStructType::getLiteral(&context, {builder.getI32Type(), ptrType});
  ...
  if (callee && !callee.isExternal() && calleeName.substr(0, 4) != "hew_") {
    // Uses the file-scope exprYieldsFieldAccess recursive scanner (defined
    // above) which covers ExprFieldAccess and all wrapper shapes: block, if,
    // if-let, match, unsafe, scope, and loop break-values.
    for (size_t i = 0; i < call.args.size(); ++i) {
    ...
    // the input stream.
    {
      ...
      for (size_t argIdx : consumedArgIndices) {
        if (argIdx >= call.args.size())
          continue;
        auto *id =
            std::get_if<ast::ExprIdentifier>(&ast::callArgExpr(call.args[argIdx]).value.kind);
        if (!id)
          continue;
...
mlir::Value MLIRGen::generatePrintCall(const ast::ExprCall &call, bool newline) {
  ...
  if (classified.named &&
    ...
    if (hasDisplayImpl) {
      ...
      // is (or wraps) a field access.  Without field-alias / partial-move
      // tracking, Display::fmt would consume the field value, then the caller's
      // scope-exit drop of the owning struct would drop it again → double-free.
      // Uses the same recursive exprYieldsFieldAccess scanner as the generic
      // call path in generateExprCall, covering direct field access and all
      // wrapper shapes (block, if, if-let, match, unsafe, scope).  The residual
      // TODO is field-alias / partial-move tracking that would let these succeed.
...
mlir::Value MLIRGen::generateStructInit(const ast::ExprStructInit &si, const ast::Span &exprSpan) {
  auto location = currentLoc;
  ...
  auto emitVecMethod = [&](mlir::Value vecValue, mlir::Type elemType,
    ...
    if (method == "map") {
      ...
      auto indirectFuncType =
          mlir::FunctionType::get(&context, indirectParamTypes, {llvmRetElemType});
      auto fnRef = hew::BitcastOp::create(builder, location, indirectFuncType, fnPtr);
    ...
    if (method == "fold") {
      ...
      auto callResult = mlir::func::CallIndirectOp::create(
          builder, location, fnRef, mlir::ValueRange{envPtr, callAcc, callElem});
...
std::optional<mlir::Value> MLIRGen::generateModuleMethodCall(const ast::ExprMethodCall &mc,
  ...
  if (ident.name == "math") {
    ...
    if (methodName == "sign") {
      ...
      auto zero =
          mlir::arith::ConstantOp::create(builder, location, i64Type, builder.getI64IntegerAttr(0));
      auto negOne = mlir::arith::ConstantOp::create(builder, location, i64Type,
                                                    builder.getI64IntegerAttr(-1));
      auto posOne =
          mlir::arith::ConstantOp::create(builder, location, i64Type, builder.getI64IntegerAttr(1));
      auto isNeg = mlir::arith::CmpIOp::create(builder, location, mlir::arith::CmpIPredicate::slt,
                                               arg, zero);
      auto isPos = mlir::arith::CmpIOp::create(builder, location, mlir::arith::CmpIPredicate::sgt,
                                               arg, zero);
...
std::optional<mlir::Value> MLIRGen::generateActorMethodCall(const ast::ExprMethodCall &mc,
                                                            mlir::Value receiver,
                                                            mlir::Location location) {
  const auto &methodName = mc.method;
  auto receiverType = receiver.getType();
...
mlir::Value MLIRGen::generateEnvDropFn(const std::vector<CapturedVarInfo> &capturedVars,
                                       mlir::LLVM::LLVMStructType envStructType,
                                       mlir::Location location) {
  auto ptrType = mlir::LLVM::LLVMPointerType::get(&context);
  ...
  for (size_t i = 0; i < capturedVars.size(); ++i) {
    if (capturedVars[i].isMutable || mlir::isa<hew::ClosureType>(capturedVars[i].value.getType()))
  ...
  auto rcDropType = builder.getFunctionType({ptrType}, {});
  ...
  for (size_t idx : rcFieldIndices) {
    auto gep = mlir::LLVM::GEPOp::create(
        ...
        llvm::ArrayRef<mlir::LLVM::GEPArg>{static_cast<int32_t>(0), static_cast<int32_t>(idx)});
    auto fieldVal = mlir::LLVM::LoadOp::create(builder, location, ptrType, gep);
    // Null-guard: the field may be zero if the lambda was only partially
    // initialised (shouldn't happen normally, but be safe).
    auto nullPtr = mlir::LLVM::ZeroOp::create(builder, location, ptrType);
    auto isNotNull = mlir::LLVM::ICmpOp::create(builder, location, builder.getI1Type(),
    ...
    mlir::func::CallOp::create(builder, location, "hew_rc_drop", mlir::TypeRange{},
...
mlir::Value MLIRGen::generateLambdaExpr(const ast::ExprLambda &lam) {
  ...
  if (lam.body) {
    ...
    fromStmtIf = [&fromBlock, &fromStmtIf](const ast::StmtIf &si, ExcludeSet &out, size_t depth) {
    ...
    fromBlock = [&collectReturnVars, &fromStmtIf](const ast::Block &blk, ExcludeSet &out,
                                                  size_t depth) {
      if (blk.trailing_expr) {
    ...
    collectReturnVars = [&collectReturnVars, &fromBlock](const ast::Expr &expr, ExcludeSet &out,
                                                         size_t depth) {
...
mlir::Value MLIRGen::generateScopeExpr(const ast::ExprScope &se, bool statementPosition) {
  auto location = currentLoc;
  auto ptrType = mlir::LLVM::LLVMPointerType::get(&context);
  ...
  if (se.block.trailing_expr) {
    ...
    bodyResult = generateTailExpr(*se.block.trailing_expr);
    // Null RAII close alloca so scope-exit drop doesn't close a
    // handle being returned out of the scope block.
    if (auto *id = std::get_if<ast::ExprIdentifier>(&se.block.trailing_expr->value.kind))
      nullOutRaiiAlloca(id->name);
  } else if (!stmts.empty()) {
    ...
    if (!hasRealTerminator(builder.getInsertionBlock())) {
      ...
      if (auto *exprStmt = std::get_if<ast::StmtExpression>(&lastStmt.kind)) {
        bodyResult = generateTailExpr(exprStmt->expr);
