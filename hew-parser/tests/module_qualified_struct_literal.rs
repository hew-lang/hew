...
// ── Regression: `fs.IoError.TimedOut(0)` remains a constructor call ──────────

/// Cross-module enum variants use the ordinary dotted postfix AST shape: the
/// final segment is a method call on the preceding field-access chain.
#[test]
...
/// NOT be parsed as a struct literal.
#[test]
fn regression_field_followed_by_block_is_not_struct_literal() {
    // The semicolon commits `obj.method`, so `{}` starts a separate block.
...
/// Deeper dotted chains are preserved in the qualified name.
#[test]
